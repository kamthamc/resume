...
export class LanguageListComponent extends Component {
  ...
  drawArcs(context) {
    let languages = this.props.languageList
      ? this.props.languageList.sort((a, b) => b.rating - a.rating).slice(0, 5)
      : [];
    const colors = {
      great: '#43A047',
      bad: '#ef5350',
    ...
    context
      ...
      .attr('fill', d =>
        d.rating < 3 ? colors.bad : d.rating < 7 ? colors.average : colors.great
      )
      .exit();

    const arcs = context.selectAll('.language-arc');
  ...
  setContext() {
    const dimension =
      2 *
        (this.arcBegins +
          (this.props.languageList.length + 1) * this.arcRadius) +
      20;
    return d3
  ...
  render() {
    return (
      <div className="language-list">
        <h4>
          <i className="fa fa-list" /> Language Skills
        </h4>
